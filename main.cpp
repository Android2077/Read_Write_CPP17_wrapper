...
(
   ...
   //---------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------------35:FindPos_substring_From_Begin:Начало--------------------------------------------------------------------------------------------
	//Это шаблонная функция - ищет в файле заданную подстроку в указанном кол-ве. Поиск идет с начала файла.
	//В качестве пути принимает кодировки: ASCII(std::string), UTF-16(std::u16string)"u", UTF-16(wstring)"L", UTF-32(std::u32string)"U" и UTF - 8(std::string)"u8" и соответвующий тип в качестве шаблона. 


	std::string substr_balvanka_52 = "Q";

	//-------------------------------------------
	const std::string path_string_UTF8_52      = u8"D:\\Мусор\\";                       //Путь до файла из которого нужно читать.
	const std::string File_Name_string_UTF8_52 = u8"FindTest.txt";                      //Имя файла из которого нужно читать.

	const char* pointer_to_FindSubstr_52       = substr_balvanka_52.c_str();            //Указатель на подстроку, которую нужно найти в файле.
	const size_t size_Split_52                 = substr_balvanka_52.size();             //Размер  подстроки, которую нужно найти в файле.
	 
	const size_t start_byte_52                 = 0;                                     //Начальный байт в файле с которого начнется поиск. Нумерация, как всегда с нуля. 0 - первый байт файла.

	const size_t number_find_52                = 3;                                     //Кол-во поисков подстроки в файле. Если нужно найти все подстроки из файла, ТО указать 0.

	std::vector<size_t>vec_pos_52;                                                      //Сюда функция номер позиции найденой подстроки в файле в конец вектора. Нумерация позиции, как всегда с нуля. 0 - это первый байт файла.
	...
	//----------------------------------------------------------------------------------------------
	//vec_pos_52.resize(0);     Так как функция добавляем результат в вектор, то если нужно чтобы результат был с начала вектора, то предварительно его обнуляем.

	const ReadWrite_FromDisk_Cpp17__class::result_flag result_52 = ReadWrite_FromDisk.FindPos_substring_From_Begin<std::string>(path_string_UTF8_52, File_Name_string_UTF8_52, pointer_to_FindSubstr_52, size_Split_52, start_byte_52, vec_pos_52, number_find_52);

	if (result_52 == ReadWrite_FromDisk_Cpp17__class::result_flag::OK)
	{
		for (int i = 0; i < vec_pos_52.size(); i++)
		{
			std::cout << "vec_pos:" << vec_pos_52[i] << std::endl;
		}
	}
	else
	{
		if (result_52 == ReadWrite_FromDisk_Cpp17__class::result_flag::NotFound)
		{
			std::cout << "NotFound" << std::endl;
		}
		else
		{
			std::cout << "error_52:" << ReadWrite_FromDisk.get__NameError() << std::endl << std::endl;
		}
	}
	//----------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------------35:FindPos_substring_From_Begin:Конец--------------------------------------------------------------------------------------------




	//----------------------------------------------------------------------------------------------------36:FindPos_substring_From_Begin:Начало--------------------------------------------------------------------------------------------

	//Это шаблонная функция - ищет в файле заданную подстроку в указанном кол-ве. Поиск идет с начала файла.
	//В качестве пути принимает кодировки: ASCII(std::string), UTF-16(std::u16string)"u", UTF-16(wstring)"L", UTF-32(std::u32string)"U" и UTF - 8(std::string)"u8" и соответвующий тип в качестве шаблона. 


	std::string substr_balvanka_53 = "Q";

	//-------------------------------------------
	const std::string Full_path_file_name_wstring_UTF8_53 = u8"D:\\Мусор\\FindTest.txt";              //Путь до файла из которого нужно читать.

	const char* pointer_to_FindSubstr_53 = substr_balvanka_53.c_str();                                //Указатель на подстроку, которую нужно найти в файле.
	const size_t size_Split_53 = substr_balvanka_53.size();                                           //Размер  подстроки, которую нужно найти в файле.

	const size_t start_byte_53 = 0;                                                                   //Начальный байт в файле с которого начнется поиск. Нумерация, как всегда с нуля. 0 - первый байт файла.

	const size_t number_find_53 = 3;                                                                  //Кол-во поисков подстроки в файле. Если нужно найти все подстроки из файла, ТО указать 0.

	std::vector<size_t>vec_pos_53;                                                                    //Сюда функция номер позиции найденой подстроки в файле в конец вектора. Нумерация позиции, как всегда с нуля. 0 - это первый байт файла.
	...
	//----------------------------------------------------------------------------------------------
	//vec_pos_53.resize(0);     Так как функция добавляем результат в вектор, то если нужно чтобы результат был с начала вектора, то предварительно его обнуляем.

	const ReadWrite_FromDisk_Cpp17__class::result_flag result_53 = ReadWrite_FromDisk.FindPos_substring_From_Begin<std::string>(Full_path_file_name_wstring_UTF8_53, pointer_to_FindSubstr_53, size_Split_53, start_byte_53, vec_pos_53, number_find_53);

	if (result_53 == ReadWrite_FromDisk_Cpp17__class::result_flag::OK)
	{
		for (int i = 0; i < vec_pos_53.size(); i++)
		{
			std::cout << "vec_pos:" << vec_pos_53[i] << std::endl;
		}
	}
	else
	{
		if (result_53 == ReadWrite_FromDisk_Cpp17__class::result_flag::NotFound)
		{
			std::cout << "NotFound" << std::endl;
		}
		else
		{
			std::cout << "error_53:" << ReadWrite_FromDisk.get__NameError() << std::endl << std::endl;
		}
	}
	//----------------------------------------------------------------------------------------------


	//----------------------------------------------------------------------------------------------------36:FindPos_substring_From_Begin:Конец--------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------------37:FindPos_substring_From_End:Начало--------------------------------------------------------------------------------------------

	//Это шаблонная функция - ищет в файле заданную подстроку в указанном кол-ве. Поиск идет с коеца файла.
	//В качестве пути принимает кодировки: ASCII(std::string), UTF-16(std::u16string)"u", UTF-16(wstring)"L", UTF-32(std::u32string)"U" и UTF - 8(std::string)"u8" и соответвующий тип в качестве шаблона. 


	std::string substr_balvanka_54 = "Q";

	//-------------------------------------------
	const std::string path_string_UTF8_54             = u8"D:\\Мусор\\";                //Путь до файла из которого нужно читать.
	const std::string File_Name_string_UTF8_54        = u8"FindTest.txt";               //Имя файла из которого нужно читать.

	const char* pointer_to_FindSubstr_54              = substr_balvanka_54.c_str();    //Указатель на подстроку, которую нужно найти в файле.
	const size_t size_Split_54                        = substr_balvanka_54.size();     //Размер  подстроки, которую нужно найти в файле.

	const size_t start_byte_54 = 0;                                                     //Начальный байт в файле с которого начнется поиск. 0 - это самый посоедний байт в файле. 1 - это предпоследний байт в файле, то есть смещение к началу файла.

	const size_t number_find_54 = 3;                                                    //Кол-во поисков подстроки в файле. Если нужно найти все подстроки из файла, ТО указать 0.

	std::vector<size_t>vec_pos_54;                                                      //Сюда функция номер позиции найденой подстроки в файле в конец вектора. Нумерация позиции, как всегда с нуля. 0 - это первый байт файла.
	...
	//----------------------------------------------------------------------------------------------
	//vec_pos_54.resize(0);     Так как функция добавляем результат в вектор, то если нужно чтобы результат был с начала вектора, то предварительно его обнуляем.

	const ReadWrite_FromDisk_Cpp17__class::result_flag result_54 = ReadWrite_FromDisk.FindPos_substring_From_End<std::string>(path_string_UTF8_54, File_Name_string_UTF8_54, pointer_to_FindSubstr_54, size_Split_54, start_byte_54, vec_pos_54, number_find_54);

	if (result_54 == ReadWrite_FromDisk_Cpp17__class::result_flag::OK)
	{
		for (int i = 0; i < vec_pos_54.size(); i++)
		{
			std::cout << "vec_pos:" << vec_pos_54[i] << std::endl;
		}
	}
	else
	{
		if (result_54 == ReadWrite_FromDisk_Cpp17__class::result_flag::NotFound)
		{
			std::cout << "NotFound" << std::endl;
		}
		else
		{
			std::cout << "error_54:" << ReadWrite_FromDisk.get__NameError() << std::endl << std::endl;
		}
	}
	//----------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------------37:FindPos_substring_From_End:Конец--------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------------38:FindPos_substring_From_End:Начало--------------------------------------------------------------------------------------------

	//Это шаблонная функция - ищет в файле заданную подстроку в указанном кол-ве. Поиск идет с коеца файла.
	//В качестве пути принимает кодировки: ASCII(std::string), UTF-16(std::u16string)"u", UTF-16(wstring)"L", UTF-32(std::u32string)"U" и UTF - 8(std::string)"u8" и соответвующий тип в качестве шаблона. 


	std::string substr_balvanka_55 = "QW";

	//-------------------------------------------
	const std::string Full_path_file_name_wstring_UTF8_55 = u8"D:\\Мусор\\FindTest.txt";                //Путь до файла из которого нужно читать.

	const char* pointer_to_FindSubstr_55 = substr_balvanka_55.c_str();                                  //Указатель на подстроку, которую нужно найти в файле.
	const size_t size_Split_55 = substr_balvanka_55.size();                                             //Размер  подстроки, которую нужно найти в файле.

	const size_t start_byte_55 = 0;                                                     //Начальный байт в файле с которого начнется поиск. 0 - это самый посоедний байт в файле. 1 - это предпоследний байт в файле, то есть смещение к началу файла.

	const size_t number_find_55 = 0;                                                    //Кол-во поисков подстроки в файле. Если нужно найти все подстроки из файла, ТО указать 0.

	std::vector<size_t>vec_pos_55;                                                      //Сюда функция номер позиции найденой подстроки в файле в конец вектора. Нумерация позиции, как всегда с нуля. 0 - это первый байт файла.
	...
	//----------------------------------------------------------------------------------------------
	//vec_pos_55.resize(0);     Так как функция добавляем результат в вектор, то если нужно чтобы результат был с начала вектора, то предварительно его обнуляем.

	const ReadWrite_FromDisk_Cpp17__class::result_flag result_55 = ReadWrite_FromDisk.FindPos_substring_From_End<std::string>(Full_path_file_name_wstring_UTF8_55, pointer_to_FindSubstr_55, size_Split_55, start_byte_55, vec_pos_55, number_find_55);

	if (result_55 == ReadWrite_FromDisk_Cpp17__class::result_flag::OK)
	{
		for (int i = 0; i < vec_pos_55.size(); i++)
		{
			std::cout << "vec_pos:" << vec_pos_55[i] << std::endl;
		}
	}
	else
	{
		if (result_55 == ReadWrite_FromDisk_Cpp17__class::result_flag::NotFound)
		{
			std::cout << "NotFound" << std::endl;
		}
		else
		{
			std::cout << "error_55:" << ReadWrite_FromDisk.get__NameError() << std::endl << std::endl;
		}
	}
	//----------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------------38:FindPos_substring_From_End:Конец--------------------------------------------------------------------------------------------
